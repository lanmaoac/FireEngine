...
{
	D3DXMATRIX R;
	//D3DXVECTOR3	 m_vup = D3DXVECTOR3(0.0f, 1.0f, 0.0f);   // 默认上向量与Y正半轴重合
	...
	m_vTargetPosition = m_vLookVector * 1;// D3DXVec3Length(&m_vCameraPosition);//更新一下观察点的新位置（方向乘以模=向量）
}
...
{
	D3DXMATRIX R;
	D3DXMatrixRotationY(&R, fAngle);//创建出绕m_vUpVector旋转fAngle个角度的R矩阵
...
VOID CameraClass::RotationLookVec(FLOAT fAngle)
{
	...
	m_vTargetPosition = m_vLookVector * 1;//更新一下观察点的新位置（方向乘以模=向量）
}
